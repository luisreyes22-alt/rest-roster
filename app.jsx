...
const VIEWS = { ADD:"add", COMPARE:"compare", ROSTER:"roster", POKEDEX:"pokedex", TEAM:"team" };
...
function spriteUrl(species) {
  ...
  return id ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/${id}.png` : null;
}
...
// A subskill slot is locked purely by the pokemon's CURRENT level. This is
// derived at read time, never trusted from storage: the stored `locked` flag
// (still written for backward compatibility) goes stale the moment a pokemon
// levels past a slot threshold.
...
// Nature modifiers, taken from nerolis-lab common/src/types/nature/nature.ts
// (not guessed): +speed natures help at a x1.1 rate, -speed at x0.925 (the
// real game is asymmetric), skill-chance natures are x1.2 / x0.8. The
// ingredient/energy/EXP axes aren't part of this score model, so natures
// touching only those stay neutral here.
...
function PokemonSprite({species, size}) {
  ...
  if (!url || broken) {
    return (
      <div style={{width:s,height:s,borderRadius:"50%",background:"var(--surface-alt)",
        ...
        flexShrink:0,fontSize:s*0.4,fontWeight:700,color:"var(--text-muted)"}}>
        {species?.[0] || "?"}
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div onClick={collapsible ? ()=>setOpen(o=>!o) : undefined}
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
          <PokemonSprite species={pokemon.species} size={collapsible && !open ? 36 : 44}/>
...
// ── Pokedex View ──────────────────────────────────────────────────────────────
function PokedexView({roster, onRemove, onEdit, onGoAdd}) {
  ...
  return (
    <div>
      <div style={{marginBottom:16}}>
        ...
        <div style={{fontSize:18,fontWeight:600}}>{speciesList.length} species · {roster.length} Pokémon</div>
      </div>
        ...
        {speciesList.map(sp => {
          ...
          return (
            <React.Fragment key={sp}>
              <div onClick={()=>setExpanded(isOpen ? null : sp)}
                style={{background:isOpen?"var(--accent-soft)":"var(--surface)",
                  border:`1px solid ${isOpen?"var(--accent)":"var(--border)"}`,
                  borderRadius:"var(--radius-control)",padding:"12px 6px 10px",cursor:"pointer",
                ...
                <PokemonSprite species={sp} size={44}/>
              ...
              {isOpen && (
                <div style={{gridColumn:"1 / -1"}}>
                  <div style={{fontSize:10,color:"var(--text-secondary)",fontFamily:"monospace",
                    margin:"4px 0 8px",letterSpacing:"0.05em"}}>
...
// Level-gated ingredient pool for a specific pokemon. Slot 1 is always the
// species' base ingredient (never stored - derived, like subskill locks).
// Slots 2/3 only exist once the pokemon reaches Lv.30/Lv.60; when the
// individual's actual roll for a slot is known (p.ingredients["30"]/["60"])
// only that ingredient counts, otherwise every option the species can roll
// there is assumed possible.
...
// Greengrass Isle (Expert Mode): berries aren't fixed - each week has 1 main favorite +
// 2 sub favorites drawn from all 18 berries, plus one random bonus. Formulas below are
// taken directly from Neroli's Lab's own expert-mode event/simulation source, not guessed.
...
function buildTeam(roster, islandName, recipeName, expertSettings) {
  ...
  // Slot 3: best Skill specialist (utility/energy)
    ...
    <div>
      ...
      {isExpertIsland && (
        <div style={{background:"var(--surface-alt)",border:"1px solid var(--border)",
          ...
          <div className="field">
            <label>Main favorite berry *</label>
            <select value={expert.mainBerry} onChange={e=>{setExpert(prev=>({...prev, mainBerry: e.target.value})); setResult(null);}}>
              <option value="">— select —</option>
              {GAME.berries.map(b => (
                <option key={b.name} value={b.name} disabled={b.name === expert.subBerry1 || b.name === expert.subBerry2}>{b.name}</option>
              ))}
            </select>
          </div>
          <div style={{display:"flex",gap:10}}>
            <div className="field" style={{flex:1}}>
              <label>Sub favorite 1 *</label>
              <select value={expert.subBerry1} onChange={e=>{setExpert(prev=>({...prev, subBerry1: e.target.value})); setResult(null);}}>
                <option value="">— select —</option>
                {GAME.berries.map(b => (
                  <option key={b.name} value={b.name} disabled={b.name === expert.mainBerry || b.name === expert.subBerry2}>{b.name}</option>
                ))}
              </select>
            </div>
            <div className="field" style={{flex:1}}>
              <label>Sub favorite 2 *</label>
              <select value={expert.subBerry2} onChange={e=>{setExpert(prev=>({...prev, subBerry2: e.target.value})); setResult(null);}}>
                <option value="">— select —</option>
                {GAME.berries.map(b => (
                  <option key={b.name} value={b.name} disabled={b.name === expert.mainBerry || b.name === expert.subBerry1}>{b.name}</option>
                ))}
              </select>
            </div>
          </div>

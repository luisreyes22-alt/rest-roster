#!/usr/bin/env node
// Regenerates luis-roster.json (the Roster > IMPORT file) from the raw
// screenshot-extraction logs. Run from anywhere:
//
//   node scripts/build-roster.cjs
//
// Inputs:  screenshots/_extraction-progress.json        (batch 1, 2026-07-04)
//          screenshots/_extraction-progress-batch2.json (batch 2, 2026-07-06)
//          gameData.json                                (validation + species facts)
// Output:  luis-roster.json (gitignored - personal data)
//
// The entry order across both batches is what fixes each pokemon's id
// ("luis_<index>_<Species>"), so never reorder or delete entries in the
// extraction files - correct them in place. Ids are how the app's import
...
const batch1 = read("screenshots/_extraction-progress.json").entries;
const batch2 = read("screenshots/_extraction-progress-batch2.json").entries;
...
if (bad > 0) {
  console.error(`\n${bad} bad reference(s) - fix the extraction files first. Nothing written.`);
  process.exit(1);
}
...
const roster = allEntries.map((e, i) => {
  ...
  return {
    id: "luis_" + i + "_" + e.species.replace(/[^a-zA-Z0-9]/g, ""),
    name: e.nickname,
    species: e.species,
...
fs.writeFileSync(path.join(ROOT, "luis-roster.json"), JSON.stringify(roster, null, 2));

const spread = roster.reduce((a, p) => { a[p.specialty] = (a[p.specialty] || 0) + 1; return a; }, {});
console.log(`Wrote ${roster.length} pokemon to luis-roster.json`);
